...
export const printers = async (id: string|null = null): Promise<Printer[]> => {
    if (id != null){
        ...
        const result: string = await invoke('plugin:printerpdf|get_printers_by_name', {
            printername
        })
    ...
    const result: string = await invoke('plugin:printerpdf|get_printers')
...
export const print = async (data: PrintData[], options: PrintOptions): Promise<ResponseResult> => {
    ...
    for (const item of data){
        if (item.type == 'image'){
            ...
            image.src = `data:${mime.getType(item.url)};base64,${Buffer.from(blob).toString('base64')}`
            if (item.width){
    ...
    const tempPath: string = await invoke('plugin:printerpdf|create_temp_file', {
    ...
    await invoke('plugin:printerpdf|print_pdf', optionsParams)
    return {
        success: true,
        message: "OK"
    }
...
export const print_file = async (options: PrintFileOptions): Promise<ResponseResult> => {
    ...
    if (typeof options.file != "undefined"){
        ...
        const filename: string = `${Math.floor(Math.random() * 100000000)}_${Date.now()}.pdf`;
        tempPath = await invoke('plugin:printerpdf|create_temp_file', {
...
export const jobs = async (printerid: string|null = null): Promise<Jobs[]> => {
    ...
    if (printerid != null){
        ...
        const result: any = await invoke('plugin:printerpdf|get_jobs', {printername: printer[0].name})
    ...
    for (const printer of listPrinter){
        const result: any = await invoke('plugin:printerpdf|get_jobs', {printername: printer.name})
        let listRawJobs: any = parseIfJSON(result, [])
        if (listRawJobs.length == undefined) listRawJobs = [listRawJobs]
        for (const job of listRawJobs){
...
export const job = async (jobid: string): Promise<Jobs|null> => {
    ...
    const result: any = await invoke('plugin:printerpdf|get_jobs_by_id', {printername: printername, jobid: id})
...
export const restart_job = async (jobid: string|null = null): Promise<ResponseResult> => {
    try {
        ...
        if (jobid != null){
            ...
            await invoke('plugin:printerpdf|restart_job', {
                printername, 
                jobid: id.toString()
            })
...
export const resume_job = async (jobid: string|null = null): Promise<ResponseResult> => {
    try {
        ...
        if (jobid != null){
            ...
            await invoke('plugin:printerpdf|resume_job', {
                printername, 
                jobid: id.toString()
            })
...
export const pause_job = async (jobid: string|null = null): Promise<ResponseResult> => {
    try {
        ...
        if (jobid != null){
            ...
            await invoke('plugin:printerpdf|pause_job', {
                printername, 
                jobid: id.toString()
            })
...
export const remove_job = async (jobid: string|null = null): Promise<ResponseResult> => {
    try {
        ...
        if (jobid != null){
            ...
            await invoke('plugin:printerpdf|remove_job', {
                printername, 
                jobid: id.toString()
            })
